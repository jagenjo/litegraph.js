...
!b.options.values)throw"LiteGraph addWidget('combo',...) requires to pass values in options: { values:['red','blue'] }";this.widgets.push(b);this.setSize(this.computeSize());return b};l.prototype.addCustomWidget=function(a){this.widgets||(this.widgets=[]);this.widgets.push(a);return a};l.prototype.getBounding=function(a){a=a||new Float32Array(4);a[0]=this.pos[0]-4;a[1]=this.pos[1]-h.NODE_TITLE_HEIGHT;a[2]=this.size[0]+4;a[3]=this.size[1]+h.NODE_TITLE_HEIGHT;if(this.onBounding)this.onBounding(a);return a};
l.prototype.isPointInside=function(a,b,c,d){c=c||0;var e=this.graph&&this.graph.isLive()?0:h.NODE_TITLE_HEIGHT;d&&(e=0);if(this.flags&&this.flags.collapsed){if(y(a,b,this.pos[0]-c,this.pos[1]-h.NODE_TITLE_HEIGHT-c,(this._collapsed_width||h.NODE_COLLAPSED_WIDTH)+2*c,h.NODE_TITLE_HEIGHT+2*c))return!0}else if(this.pos[0]-4-c<a&&this.pos[0]+this.size[0]+4+c>a&&this.pos[1]-e-c<b&&this.pos[1]+this.size[1]+c>b)return!0;return!1};l.prototype.getSlotInPosition=function(a,b){var c=new Float32Array(2);if(this.inputs)for(var d=
0,e=this.inputs.length;d<e;++d){var g=this.inputs[d];this.getConnectionPos(!0,d,c);if(y(a,b,c[0]-10,c[1]-5,20,10))return{input:g,slot:d,link_pos:c}}if(this.outputs)for(d=0,e=this.outputs.length;d<e;++d)if(g=this.outputs[d],this.getConnectionPos(!1,d,c),y(a,b,c[0]-10,c[1]-5,20,10))return{output:g,slot:d,link_pos:c};return null};l.prototype.findInputSlot=function(a){if(!this.inputs)return-1;for(var b=0,c=this.inputs.length;b<c;++b)if(a==this.inputs[b].name)return b;return-1};l.prototype.findOutputSlot=
function(a){if(!this.outputs)return-1;for(var b=0,c=this.outputs.length;b<c;++b)if(a==this.outputs[b].name)return b;return-1};l.prototype.connect=function(a,b,c){c=c||0;if(!this.graph)return console.log("Connect: Error, node doesn't belong to any graph. Nodes must be added first to a graph before connecting them."),null;if(a.constructor===String){if(a=this.findOutputSlot(a),-1==a)return h.debug&&console.log("Connect: Error, no slot of name "+a),null}else if(!this.outputs||a>=this.outputs.length)return h.debug&&
...
d=!0);var e=this.outputs[a];if(b.onConnectInput&&!1===b.onConnectInput(c,e.type,e,this,a))return null;var g=b.inputs[c],f=null;if(!h.isValidConnection(e.type,g.type))return this.setDirtyCanvas(!1,!0),d&&this.graph.connectionChange(this,f),null;d||this.graph.beforeChange();f=new u(++this.graph.last_link_id,g.type,this.id,a,b.id,c);this.graph.links[f.id]=f;null==e.links&&(e.links=[]);e.links.push(f.id);b.inputs[c].link=f.id;this.graph&&this.graph._version++;if(this.onConnectionsChange)this.onConnectionsChange(h.OUTPUT,
a,!0,f,e);if(b.onConnectionsChange)b.onConnectionsChange(h.INPUT,c,!0,f,g);this.graph&&this.graph.onNodeConnectionChange&&(this.graph.onNodeConnectionChange(h.INPUT,b,c,this,a),this.graph.onNodeConnectionChange(h.OUTPUT,this,a,b,c));this.setDirtyCanvas(!1,!0);this.graph.afterChange();this.graph.connectionChange(this,f);return f};l.prototype.disconnectOutput=function(a,b){if(a.constructor===String){if(a=this.findOutputSlot(a),-1==a)return h.debug&&console.log("Connect: Error, no slot of name "+a),
!1}else if(!this.outputs||a>=this.outputs.length)return h.debug&&console.log("Connect: Error, slot number not found"),!1;var c=this.outputs[a];if(!c||!c.links||0==c.links.length)return!1;if(b){b.constructor===Number&&(b=this.graph.getNodeById(b));if(!b)throw"Target Node not found";for(var d=0,e=c.links.length;d<e;d++){var g=c.links[d],f=this.graph.links[g];if(f.target_id==b.id){c.links.splice(d,1);var m=b.inputs[f.target_slot];m.link=null;delete this.graph.links[g];this.graph&&this.graph._version++;
...
function(){this._nodes.length=0;for(var a=this.graph._nodes,b=new Float32Array(4),c=0;c<a.length;++c){var d=a[c];d.getBounding(b);A(this._bounding,b)&&this._nodes.push(d)}};r.prototype.isPointInside=l.prototype.isPointInside;r.prototype.setDirtyCanvas=l.prototype.setDirtyCanvas;h.DragAndScale=v;v.prototype.bindEvents=function(a){this.last_mouse=new Float32Array(2);this._binded_mouse_callback=this.onMouse.bind(this);a.addEventListener("mousedown",this._binded_mouse_callback);a.addEventListener("mousemove",
...
this.canvas.removeEventListener("touchcancel",this.touchHandler);this._ondrop_callback=this._key_callback=this._mousewheel_callback=this._mousedown_callback=null;this._events_binded=!1}else console.warn("LGraphCanvas: no events binded")};k.getFileExtension=function(a){var b=a.indexOf("?");-1!=b&&(a=a.substr(0,b));b=a.lastIndexOf(".");return-1==b?"":a.substr(b+1).toLowerCase()};k.prototype.enableWebGL=function(){if(void 0===typeof GL)throw"litegl.js must be included to use a WebGL canvas";if(void 0===
typeof enableWebGLCanvas)throw"webglCanvas.js must be included to use this feature";this.gl=this.ctx=enableWebGLCanvas(this.canvas);this.ctx.webgl=!0;this.bgcanvas=this.canvas;this.bgctx=this.gl;this.canvas.webgl_enabled=!0};k.prototype.setDirty=function(a,b){a&&(this.dirty_canvas=!0);b&&(this.dirty_bgcanvas=!0)};k.prototype.getCanvasWindow=function(){if(!this.canvas)return window;var a=this.canvas.ownerDocument;return a.defaultView||a.parentWindow};k.prototype.startRendering=function(){function a(){this.pause_rendering||
...
p.last_y+w)){d=p.value;switch(p.type){case "button":if("mousemove"===c.type)break;p.callback&&setTimeout(function(){p.callback(p,q,a,b,c)},20);this.dirty_canvas=p.clicked=!0;break;case "slider":k=Math.clamp((g-15)/(t-30),0,1);p.value=p.options.min+(p.options.max-p.options.min)*k;p.callback&&setTimeout(function(){e(p,p.value)},20);this.dirty_canvas=!0;break;case "number":case "combo":d=p.value;if("mousemove"==c.type&&"number"==p.type)p.value+=0.1*c.deltaX*(p.options.step||1),null!=p.options.min&&p.value<
p.options.min&&(p.value=p.options.min),null!=p.options.max&&p.value>p.options.max&&(p.value=p.options.max);else if("mousedown"==c.type){var r=p.options.values;r&&r.constructor===Function&&(r=p.options.values(p,a));var n=null;"number"!=p.type&&(n=r.constructor===Array?r:Object.keys(r));g=40>g?-1:g>t-40?1:0;if("number"==p.type)p.value+=0.1*g*(p.options.step||1),null!=p.options.min&&p.value<p.options.min&&(p.value=p.options.min),null!=p.options.max&&p.value>p.options.max&&(p.value=p.options.max);else if(g)k=
-1,this.last_mouseclick=0,k=r.constructor===Object?n.indexOf(String(p.value))+g:n.indexOf(p.value)+g,k>=n.length&&(k=n.length-1),0>k&&(k=0),p.value=r.constructor===Array?r[k]:k;else{var v=r!=n?Object.values(r):r;new h.ContextMenu(v,{scale:Math.max(1,this.ds.scale),event:c,className:"dark",callback:u.bind(p)},k);var u=function(a,b,c){r!=n&&(a=v.indexOf(a));this.value=a;e(this,a);q.dirty_canvas=!0;return!1}}}else"mouseup"==c.type&&"number"==p.type&&(g=40>g?-1:g>t-40?1:0,200>c.click_time&&0==g&&this.prompt("Value",
p.value,function(a){this.value=Number(a);e(this,this.value)}.bind(p),c));d!=p.value&&setTimeout(function(){e(this,this.value)}.bind(p),20);this.dirty_canvas=!0;break;case "toggle":"mousedown"==c.type&&(p.value=!p.value,setTimeout(function(){e(p,p.value)},20));break;case "string":case "text":"mousedown"==c.type&&this.prompt("Value",p.value,function(a){this.value=a;e(this,a)}.bind(p),c,p.options?p.options.multiline:!1);break;default:p.mouse&&(this.dirty_canvas=p.mouse(c,[g,f],a))}if(d!=p.value){if(a.onWidgetChanged)a.onWidgetChanged(p.name,
p.value,d,p);a.graph._version++}return p}}}return null};k.prototype.drawGroups=function(a,b){if(this.graph){var c=this.graph._groups;b.save();b.globalAlpha=0.5*this.editor_alpha;for(var d=0;d<c.length;++d){var e=c[d];if(A(this.visible_area,e._bounding)){b.fillStyle=e.color||"#335";b.strokeStyle=e.color||"#335";var g=e._pos,f=e._size;b.globalAlpha=0.25*this.editor_alpha;b.beginPath();b.rect(g[0]+0.5,g[1]+0.5,f[0],f[1]);b.fill();b.globalAlpha=this.editor_alpha;b.stroke();b.beginPath();b.moveTo(g[0]+
f[0],g[1]+f[1]);b.lineTo(g[0]+f[0]-10,g[1]+f[1]);b.lineTo(g[0]+f[0],g[1]+f[1]-10);b.fill();f=e.font_size||h.DEFAULT_GROUP_FONT_SIZE;b.font=f+"px Arial";b.fillText(e.title,g[0]+4,g[1]+f)}}b.restore()}};k.prototype.adjustNodesSize=function(){for(var a=this.graph._nodes,b=0;b<a.length;++b)a[b].size=a[b].computeSize();this.setDirty(!0,!0)};k.prototype.resize=function(a,b){if(!a&&!b){var c=this.canvas.parentNode;a=c.offsetWidth;b=c.offsetHeight}if(this.canvas.width!=a||this.canvas.height!=b)this.canvas.width=
a,this.canvas.height=b,this.bgcanvas.width=this.canvas.width,this.bgcanvas.height=this.canvas.height,this.setDirty(!0,!0)};k.prototype.switchLiveMode=function(a){if(a){var b=this,c=this.live_mode?1.1:0.9;this.live_mode&&(this.live_mode=!1,this.editor_alpha=0.1);var d=setInterval(function(){b.editor_alpha*=c;b.dirty_canvas=!0;b.dirty_bgcanvas=!0;1>c&&0.01>b.editor_alpha&&(clearInterval(d),1>c&&(b.live_mode=!0));1<c&&0.99<b.editor_alpha&&(clearInterval(d),b.editor_alpha=1)},1)}else this.live_mode=!this.live_mode,
this.dirty_bgcanvas=this.dirty_canvas=!0};k.prototype.onNodeSelectionChange=function(a){};k.prototype.touchHandler=function(a){var b=a.changedTouches[0],c="";switch(a.type){case "touchstart":c="mousedown";break;case "touchmove":c="mousemove";break;case "touchend":c="mouseup";break;default:return}var d=this.getCanvasWindow(),e=d.document.createEvent("MouseEvent");e.initMouseEvent(c,!0,!0,d,1,b.screenX,b.screenY,b.clientX,b.clientY,!1,!1,!1,!1,0,null);b.target.dispatchEvent(e);a.preventDefault()};k.onGroupAdd=
function(a,b,c){a=k.active_canvas;a.getCanvasWindow();b=new h.LGraphGroup;b.pos=a.convertEventToCanvasOffset(c);a.graph.add(b)};k.onMenuAdd=function(a,b,c,d,e){function g(a,b){var d=[];h.getNodeTypesCategories(f.filter||q.filter).filter(function(b){return b.startsWith(a)}).map(function(b){if(b){b=b.replace(RegExp("^("+a+")"),"").split("/")[0];var c=""===a?b+"/":a+b+"/";-1!=b.indexOf("::")&&(b=b.split("::")[1]);-1===d.findIndex(function(a){return a.value===c})&&d.push({value:c,content:b,has_submenu:!0,
callback:function(a,b,c,e){g(a.value,e)}})}});h.getNodeTypesInCategory(a.slice(0,-1),f.filter||q.filter).map(function(a){a.skip_list||d.push({value:a.type,content:a.title,has_submenu:!1,callback:function(a,b,c,d){b=d.getFirstEvent();f.graph.beforeChange();if(a=h.createNode(a.value))a.pos=f.convertEventToCanvasOffset(b),f.graph.add(a);e&&e(a);f.graph.afterChange()}})});new h.ContextMenu(d,{event:c,parentMenu:b},m)}var f=k.active_canvas,m=f.getCanvasWindow(),q=f.graph;if(q)return g("",d),!1};k.onMenuCollapseAll=
function(){};k.onMenuNodeEdit=function(){};k.showMenuNodeOptionalInputs=function(a,b,c,d,e){function g(a,b,c){e&&(a.callback&&a.callback.call(f,e,a,b,c),a.value&&(e.graph.beforeChange(),e.addInput(a.value[0],a.value[1],a.value[2]),e.setDirtyCanvas(!0,!0),e.graph.afterChange()))}if(e){var f=this;a=k.active_canvas.getCanvasWindow();b=e.optional_inputs;e.onGetInputs&&(b=e.onGetInputs());var m=[];if(b)for(var q=0;q<b.length;q++){var l=b[q];if(l){var r=l[0];l[2]&&l[2].label&&(r=l[2].label);r={content:r,
value:l};l[1]==h.ACTION&&(r.className="event");m.push(r)}else m.push(null)}this.onMenuNodeInputs&&(m=this.onMenuNodeInputs(m));if(m.length)return new h.ContextMenu(m,{event:c,callback:g,parentMenu:d,node:e},a),!1;console.log("no input entries")}};k.showMenuNodeOptionalOutputs=function(a,b,c,d,e){function g(a,b,c){if(e&&(a.callback&&a.callback.call(f,e,a,b,c),a.value))if(c=a.value[1],!c||c.constructor!==Object&&c.constructor!==Array)e.graph.beforeChange(),e.addOutput(a.value[0],a.value[1],a.value[2]),
e.setDirtyCanvas(!0,!0),e.graph.afterChange();else{a=[];for(var m in c)a.push({content:m,value:c[m]});new h.ContextMenu(a,{event:b,callback:g,parentMenu:d,node:e});return!1}}if(e){var f=this;a=k.active_canvas.getCanvasWindow();b=e.optional_outputs;e.onGetOutputs&&(b=e.onGetOutputs());var m=[];if(b)for(var q=0;q<b.length;q++){var l=b[q];if(!l)m.push(null);else if(!e.flags||!e.flags.skip_repeated_outputs||-1==e.findOutputSlot(l[0])){var r=l[0];l[2]&&l[2].label&&(r=l[2].label);r={content:r,value:l};
l[1]==h.EVENT&&(r.className="event");m.push(r)}}this.onMenuNodeOutputs&&(m=this.onMenuNodeOutputs(m));if(m.length)return new h.ContextMenu(m,{event:c,callback:g,parentMenu:d,node:e},a),!1}};k.onShowMenuNodeProperties=function(a,b,c,d,e){function g(a,b,c,d){e&&(b=this.getBoundingClientRect(),f.showEditPropertyValue(e,a.value,{position:[b.left,b.top]}))}if(e&&e.properties){var f=k.active_canvas;b=f.getCanvasWindow();var m=[],q;for(q in e.properties){a=void 0!==e.properties[q]?e.properties[q]:" ";"object"==
typeof a&&(a=JSON.stringify(a));var l=e.getPropertyInfo(q);if("enum"==l.type||"combo"==l.type)a=k.getPropertyPrintableValue(a,l.values);a=k.decodeHTML(a);m.push({content:"<span class='property_name'>"+(l.label?l.label:q)+"</span><span class='property_value'>"+a+"</span>",value:q})}if(m.length)return new h.ContextMenu(m,{event:c,callback:g,parentMenu:d,allow_html:!0,node:e},b),!1}};k.decodeHTML=function(a){var b=document.createElement("div");b.innerText=a;return b.innerHTML};k.onResizeNode=function(a,
b,c,d,e){if(e){e.size=e.computeSize();if(e.onResize)e.onResize(e.size);e.setDirtyCanvas(!0,!0)}};k.prototype.showLinkMenu=function(a,b){var c=this;console.log(a);var d=new h.ContextMenu(["Add Node",null,"Delete"],{event:b,title:null!=a.data?a.data.constructor.name:null,callback:function(b,g,f){switch(b){case "Add Node":k.onMenuAdd(null,null,f,d,function(b){console.log("node autoconnect");var e=c.graph.getNodeById(a.origin_id),d=c.graph.getNodeById(a.target_id);b.inputs&&b.inputs.length&&b.outputs&&
b.outputs.length&&e.outputs[a.origin_slot].type==b.inputs[0].type&&b.outputs[0].type==d.inputs[0].type&&(e.connect(a.origin_slot,b,0),b.connect(0,d,a.target_slot),b.pos[0]-=0.5*b.size[0])});break;case "Delete":c.graph.removeLink(a.id)}}});return!1};k.onShowPropertyEditor=function(a,b,c,d,e){function g(){var b=q.value;"Number"==a.type?b=Number(b):"Boolean"==a.type&&(b=Boolean(b));e[f]=b;m.parentNode&&m.parentNode.removeChild(m);e.setDirtyCanvas(!0,!0)}var f=a.property||"title";b=e[f];var m=document.createElement("div");
m.className="graphdialog";m.innerHTML="<span class='name'></span><input autofocus type='text' class='value'/><button>OK</button>";m.querySelector(".name").innerText=f;var q=m.querySelector(".value");q&&(q.value=b,q.addEventListener("blur",function(a){this.focus()}),q.addEventListener("keydown",function(a){if(13==a.keyCode||"textarea"==a.target.localName)g(),a.preventDefault(),a.stopPropagation()}));b=k.active_canvas.canvas;c=b.getBoundingClientRect();var h=d=-20;c&&(d-=c.left,h-=c.top);event?(m.style.left=
event.clientX+d+"px",m.style.top=event.clientY+h+"px"):(m.style.left=0.5*b.width+d+"px",m.style.top=0.5*b.height+h+"px");m.querySelector("button").addEventListener("click",g);b.parentNode.appendChild(m)};k.prototype.prompt=function(a,b,c,d,e){var g=this;a=a||"";var f=!1,m=document.createElement("div");m.className="graphdialog rounded";m.innerHTML=e?"<span class='name'></span> <textarea autofocus class='value'></textarea><button class='rounded'>OK</button>":"<span class='name'></span> <input autofocus type='text' class='value'/><button class='rounded'>OK</button>";
m.close=function(){g.prompt_box=null;m.parentNode&&m.parentNode.removeChild(m)};1<this.ds.scale&&(m.style.transform="scale("+this.ds.scale+")");m.addEventListener("mouseleave",function(a){f||m.close()});g.prompt_box&&g.prompt_box.close();g.prompt_box=m;m.querySelector(".name").innerText=a;var q=m.querySelector(".value");q.value=b;q.addEventListener("keydown",function(a){f=!0;if(27==a.keyCode)m.close();else if(13==a.keyCode&&"textarea"!=a.target.localName)c&&c(this.value),m.close();else return;a.preventDefault();
a.stopPropagation()});m.querySelector("button").addEventListener("click",function(a){c&&c(q.value);g.setDirty(!0);m.close()});a=k.active_canvas.canvas;b=a.getBoundingClientRect();var h=e=-20;b&&(e-=b.left,h-=b.top);d?(m.style.left=d.clientX+e+"px",m.style.top=d.clientY+h+"px"):(m.style.left=0.5*a.width+e+"px",m.style.top=0.5*a.height+h+"px");a.parentNode.appendChild(m);setTimeout(function(){q.focus()},10);return m};k.search_limit=-1;k.prototype.showSearchBox=function(a){function b(b){if(b)if(e.onSearchBoxSelection)e.onSearchBoxSelection(b,
a,g);else{var c=h.searchbox_extras[b.toLowerCase()];c&&(b=c.type);g.graph.beforeChange();if(b=h.createNode(b))b.pos=g.convertEventToCanvasOffset(a),g.graph.add(b);if(c&&c.data){if(c.data.properties)for(var d in c.data.properties)b.addProperty(d,c.data.properties[d]);if(c.data.inputs)for(d in b.inputs=[],c.data.inputs)b.addOutput(c.data.inputs[d][0],c.data.inputs[d][1]);if(c.data.outputs)for(d in b.outputs=[],c.data.outputs)b.addOutput(c.data.outputs[d][0],c.data.outputs[d][1]);c.data.title&&(b.title=
c.data.title);c.data.json&&b.configure(c.data.json);g.graph.afterChange()}}q.close()}function c(a){var b=t;t&&t.classList.remove("selected");t?(t=a?t.nextSibling:t.previousSibling)||(t=b):t=a?r.childNodes[0]:r.childNodes[r.childNodes.length];t&&(t.classList.add("selected"),t.scrollIntoView({block:"end",behavior:"smooth"}))}function d(){function a(c,d){var e=document.createElement("div");p||(p=c);e.innerText=c;e.dataset.type=escape(c);e.className="litegraph lite-search-item";d&&(e.className+=" "+d);
e.addEventListener("click",function(a){b(unescape(this.dataset.type))});r.appendChild(e)}w=null;var c=J.value;p=null;r.innerHTML="";if(c)if(e.onSearchBox){var d=e.onSearchBox(r,c,g);if(d)for(var m=0;m<d.length;++m)a(d[m])}else{var d=0,c=c.toLowerCase(),f=g.filter||g.graph.filter;for(m in h.searchbox_extras){var q=h.searchbox_extras[m];if(-1!==q.desc.toLowerCase().indexOf(c)){var l=h.registered_node_types[q.type];if(!l||l.filter==f)if(a(q.desc,"searchbox_extra"),-1!==k.search_limit&&d++>k.search_limit)break}}q=
null;if(Array.prototype.filter)q=Object.keys(h.registered_node_types).filter(t);else for(m in q=[],h.registered_node_types)t(m)&&q.push(m);for(m=0;m<q.length&&!(a(q[m]),-1!==k.search_limit&&d++>k.search_limit);m++);var t=function(a){var b=h.registered_node_types[a];return f&&b.filter!=f?!1:-1!==a.toLowerCase().indexOf(c)}}}var e=this,g=k.active_canvas,f=g.canvas,m=f.ownerDocument||document,q=document.createElement("div");q.className="litegraph litesearchbox graphdialog rounded";q.innerHTML="<span class='name'>Search</span> <input autofocus type='text' class='value rounded'/><div class='helper'></div>";
q.close=function(){e.search_box=null;m.body.focus();m.body.style.overflow="";setTimeout(function(){e.canvas.focus()},20);q.parentNode&&q.parentNode.removeChild(q)};var l=null;1<this.ds.scale&&(q.style.transform="scale("+this.ds.scale+")");q.addEventListener("mouseenter",function(a){l&&(clearTimeout(l),l=null)});q.addEventListener("mouseleave",function(a){l=setTimeout(function(){q.close()},500)});e.search_box&&e.search_box.close();e.search_box=q;var r=q.querySelector(".helper"),p=null,w=null,t=null,
J=q.querySelector("input");J&&(J.addEventListener("blur",function(a){this.focus()}),J.addEventListener("keydown",function(a){if(38==a.keyCode)c(!1);else if(40==a.keyCode)c(!0);else if(27==a.keyCode)q.close();else if(13==a.keyCode)t?b(t.innerHTML):p?b(p):q.close();else{w&&clearInterval(w);w=setTimeout(d,10);return}a.preventDefault();a.stopPropagation();a.stopImmediatePropagation();return!0}));m.fullscreenElement?m.fullscreenElement.appendChild(q):(m.body.appendChild(q),m.body.style.overflow="hidden");
var f=f.getBoundingClientRect(),n=(a?a.clientY:f.top+0.5*f.height)-20;q.style.left=(a?a.clientX:f.left+0.5*f.width)-80+"px";q.style.top=n+"px";a.layerY>f.height-200&&(r.style.maxHeight=f.height-a.layerY-20+"px");J.focus();return q};k.prototype.showEditPropertyValue=function(a,b,c){function d(){e(p.value)}function e(d){g&&g.values&&g.values.constructor===Object&&void 0!=g.values[d]&&(d=g.values[d]);"number"==typeof a.properties[b]&&(d=Number(d));if("array"==f||"object"==f)d=JSON.parse(d);a.properties[b]=
d;a.graph&&a.graph._version++;if(a.onPropertyChanged)a.onPropertyChanged(b,d);if(c.onclose)c.onclose();k.close();a.setDirtyCanvas(!0,!0)}if(a&&void 0!==a.properties[b]){c=c||{};var g=a.getPropertyInfo(b),f=g.type,m="";if("string"==f||"number"==f||"array"==f||"object"==f)m="<input autofocus type='text' class='value'/>";else if("enum"!=f&&"combo"!=f||!g.values)if("boolean"==f)m="<input autofocus type='checkbox' class='value' "+(a.properties[b]?"checked":"")+"/>";else{console.warn("unknown type: "+f);
return}else{var m="<select autofocus type='text' class='value'>",q;for(q in g.values){var h=q;g.values.constructor===Array&&(h=g.values[q]);m+="<option value='"+h+"' "+(h==a.properties[b]?"selected":"")+">"+g.values[q]+"</option>"}m+="</select>"}var k=this.createDialog("<span class='name'>"+(g.label?g.label:b)+"</span>"+m+"<button>OK</button>",c);if("enum"!=f&&"combo"!=f||!g.values)if("boolean"==f)(p=k.querySelector("input"))&&p.addEventListener("click",function(a){e(!!p.checked)});else{if(p=k.querySelector("input"))p.addEventListener("blur",
function(a){this.focus()}),h=void 0!==a.properties[b]?a.properties[b]:"","string"!==f&&(h=JSON.stringify(h)),p.value=h,p.addEventListener("keydown",function(a){13==a.keyCode&&(d(),a.preventDefault(),a.stopPropagation())})}else{var p=k.querySelector("select");p.addEventListener("change",function(a){e(a.target.value)})}k.querySelector("button").addEventListener("click",d);return k}};k.prototype.createDialog=function(a,b){b=b||{};var c=document.createElement("div");c.className="graphdialog";c.innerHTML=
a;var d=this.canvas.getBoundingClientRect(),e=-20,g=-20;d&&(e-=d.left,g-=d.top);b.position?(e+=b.position[0],g+=b.position[1]):b.event?(e+=b.event.clientX,g+=b.event.clientY):(e+=0.5*this.canvas.width,g+=0.5*this.canvas.height);c.style.left=e+"px";c.style.top=g+"px";this.canvas.parentNode.appendChild(c);c.close=function(){this.parentNode&&this.parentNode.removeChild(this)};return c};k.prototype.createPanel=function(a,b){b=b||{};var c=b.window||window,d=document.createElement("div");d.className="litegraph dialog";
d.innerHTML="<div class='dialog-header'><span class='dialog-title'></span></div><div class='dialog-content'></div><div class='dialog-footer'></div>";d.header=d.querySelector(".dialog-header");b.width&&(d.style.width=b.width+(b.width.constructor===Number?"px":""));b.height&&(d.style.height=b.height+(b.height.constructor===Number?"px":""));if(b.closable){var e=document.createElement("span");e.innerHTML="&#10005;";e.classList.add("close");e.addEventListener("click",function(){d.close()});d.header.appendChild(e)}d.title_element=
d.querySelector(".dialog-title");d.title_element.innerText=a;d.content=d.querySelector(".dialog-content");d.footer=d.querySelector(".dialog-footer");d.close=function(){this.parentNode&&this.parentNode.removeChild(this)};d.clear=function(){this.content.innerHTML=""};d.addHTML=function(a,b,c){var e=document.createElement("div");b&&(e.className=b);e.innerHTML=a;c?d.footer.appendChild(e):d.content.appendChild(e);return e};d.addButton=function(a,b,c){var e=document.createElement("button");e.innerText=
a;e.options=c;e.classList.add("btn");e.addEventListener("click",b);d.footer.appendChild(e);return e};d.addSeparator=function(){var a=document.createElement("div");a.className="separator";d.content.appendChild(a)};d.addWidget=function(a,b,e,q,l){function r(a,b){console.log("change",a,b);q.callback&&q.callback(a,b);l&&l(a,b)}q=q||{};var p=String(e);a=a.toLowerCase();"number"==a&&(p=e.toFixed(3));var w=document.createElement("div");w.className="property";w.innerHTML="<span class='property_name'></span><span class='property_value'></span>";
w.querySelector(".property_name").innerText=b;var t=w.querySelector(".property_value");t.innerText=p;w.dataset.property=b;w.dataset.type=q.type||a;w.options=q;w.value=e;if("boolean"==a)w.classList.add("boolean"),e&&w.classList.add("bool-on"),w.addEventListener("click",function(){var a=this.dataset.property;this.value=!this.value;this.classList.toggle("bool-on");this.querySelector(".property_value").innerText=this.value?"true":"false";r(a,this.value)});else if("string"==a||"number"==a)t.setAttribute("contenteditable",
!0),t.addEventListener("keydown",function(a){"Enter"==a.code&&(a.preventDefault(),this.blur())}),t.addEventListener("blur",function(){var a=this.innerText,b=this.parentNode.dataset.property;"number"==this.parentNode.dataset.type&&(a=Number(a));r(b,a)});else if("enum"==a||"combo"==a)p=k.getPropertyPrintableValue(e,q.values);t.innerText=p;t.addEventListener("click",function(a){var b=this.parentNode.dataset.property,e=this;new h.ContextMenu(q.values||[],{event:a,className:"dark",callback:function(a,
...
!0})}b.addSeparator();if(a.onShowCustomPanelInfo)a.onShowCustomPanelInfo(b);b.addButton("Delete",function(){a.block_delete||(a.graph.remove(a),b.close())}).classList.add("delete")})();this.canvas.parentNode.appendChild(b)};k.prototype.showSubgraphPropertiesDialog=function(a){function b(){d.clear();if(a.inputs)for(var c=0;c<a.inputs.length;++c){var g=a.inputs[c];if(!g.not_subgraph_input){var f=d.addHTML("<button>&#10005;</button> <span class='bullet_icon'></span><span class='name'></span><span class='type'></span>",
...
bgcolor:"#535",groupcolor:"#a1309b"},yellow:{color:"#432",bgcolor:"#653",groupcolor:"#b58b2a"},black:{color:"#222",bgcolor:"#000",groupcolor:"#444"}};k.prototype.getCanvasMenuOptions=function(){var a=null;this.getMenuOptions?a=this.getMenuOptions():(a=[{content:"Add Node",has_submenu:!0,callback:k.onMenuAdd},{content:"Add Group",callback:k.onGroupAdd}],this._graph_stack&&0<this._graph_stack.length&&a.push(null,{content:"Close subgraph",callback:this.closeSubgraph.bind(this)}));if(this.getExtraMenuOptions){var b=
this.getExtraMenuOptions(this,a);b&&(a=a.concat(b))}return a};k.prototype.getNodeMenuOptions=function(a){var b=null,b=a.getMenuOptions?a.getMenuOptions(this):[{content:"Inputs",has_submenu:!0,disabled:!0,callback:k.showMenuNodeOptionalInputs},{content:"Outputs",has_submenu:!0,disabled:!0,callback:k.showMenuNodeOptionalOutputs},null,{content:"Properties",has_submenu:!0,callback:k.onShowMenuNodeProperties},null,{content:"Title",callback:k.onShowPropertyEditor},{content:"Mode",has_submenu:!0,callback:k.onMenuNodeMode},
{content:"Resize",callback:function(){if(a.resizable)return k.onResizeNode}},{content:"Collapse",callback:k.onMenuNodeCollapse},{content:"Pin",callback:k.onMenuNodePin},{content:"Colors",has_submenu:!0,callback:k.onMenuNodeColors},{content:"Shapes",has_submenu:!0,callback:k.onMenuNodeShapes},null];if(a.onGetInputs){var c=a.onGetInputs();c&&c.length&&(b[0].disabled=!1)}a.onGetOutputs&&(c=a.onGetOutputs())&&c.length&&(b[1].disabled=!1);a.getExtraMenuOptions&&(c=a.getExtraMenuOptions(this,b))&&(c.push(null),
b=c.concat(b));!1!==a.clonable&&b.push({content:"Clone",callback:k.onMenuNodeClone});b.push(null,{content:"Remove",disabled:!(!1!==a.removable&&!a.block_delete),callback:k.onMenuNodeRemove});if(a.graph&&a.graph.onGetNodeMenuOptions)a.graph.onGetNodeMenuOptions(b,a);return b};k.prototype.getGroupMenuOptions=function(a){return[{content:"Title",callback:k.onShowPropertyEditor},{content:"Color",has_submenu:!0,callback:k.onMenuNodeColors},{content:"Font size",property:"font_size",type:"Number",callback:k.onShowPropertyEditor},
null,{content:"Remove",callback:k.onMenuNodeRemove}]};k.prototype.processContextMenu=function(a,b){var c=this,d=k.active_canvas.getCanvasWindow(),e=null,g={event:b,callback:function(b,d,e){if(b)if("Remove Slot"==b.content)b=b.slot,b.input?a.removeInput(b.slot):b.output&&a.removeOutput(b.slot);else if("Disconnect Links"==b.content)b=b.slot,b.output?a.disconnectOutput(b.slot):b.input&&a.disconnectInput(b.slot);else if("Rename Slot"==b.content){b=b.slot;var m=b.input?a.getInputInfo(b.slot):a.getOutputInfo(b.slot),
f=c.createDialog("<span class='name'>Name</span><input autofocus type='text'/><button>OK</button>",d),g=f.querySelector("input");g&&m&&(g.value=m.label||"");f.querySelector("button").addEventListener("click",function(a){g.value&&(m&&(m.label=g.value),c.setDirty(!0));f.close()})}},extra:a};a&&(g.title=a.type);var f=null;a&&(f=a.getSlotInPosition(b.canvasX,b.canvasY),k.active_node=a);if(f){e=[];if(a.getSlotMenuOptions)e=a.getSlotMenuOptions(f);else{f&&f.output&&f.output.links&&f.output.links.length&&
e.push({content:"Disconnect Links",slot:f});var m=f.input||f.output;e.push(m.locked?"Cannot remove":{content:"Remove Slot",slot:f});e.push(m.nameLocked?"Cannot rename":{content:"Rename Slot",slot:f})}g.title=(f.input?f.input.type:f.output.type)||"*";f.input&&f.input.type==h.ACTION&&(g.title="Action");f.output&&f.output.type==h.EVENT&&(g.title="Event")}else a?e=this.getNodeMenuOptions(a):(e=this.getCanvasMenuOptions(),(f=this.graph.getGroupOnPos(b.canvasX,b.canvasY))&&e.push(null,{content:"Edit Group",
has_submenu:!0,submenu:{title:"Group",extra:f,options:this.getGroupMenuOptions(f)}}));e&&new h.ContextMenu(e,g,d)};"undefined"!=typeof window&&window.CanvasRenderingContext2D&&(window.CanvasRenderingContext2D.prototype.roundRect=function(a,b,c,d,e,g){void 0===e&&(e=5);void 0===g&&(g=e);this.moveTo(a+e,b);this.lineTo(a+c-e,b);this.quadraticCurveTo(a+c,b,a+c,b+e);this.lineTo(a+c,b+d-g);this.quadraticCurveTo(a+c,b+d,a+c-g,b+d);this.lineTo(a+g,b+d);this.quadraticCurveTo(a,b+d,a,b+d-g);this.lineTo(a,b+
e);this.quadraticCurveTo(a,b,a+e,b)});h.compareObjects=function(a,b){for(var c in a)if(a[c]!=b[c])return!1;return!0};h.distance=E;h.colorToString=function(a){return"rgba("+Math.round(255*a[0]).toFixed()+","+Math.round(255*a[1]).toFixed()+","+Math.round(255*a[2]).toFixed()+","+(4==a.length?a[3].toFixed(2):"1.0")+")"};h.isInsideRectangle=y;h.growBounding=function(a,b,c){b<a[0]?a[0]=b:b>a[2]&&(a[2]=b);c<a[1]?a[1]=c:c>a[3]&&(a[3]=c)};h.isInsideBounding=function(a,b){return a[0]<b[0][0]||a[1]<b[0][1]||
a[0]>b[1][0]||a[1]>b[1][1]?!1:!0};h.overlapBounding=A;h.hex2num=function(a){"#"==a.charAt(0)&&(a=a.slice(1));a=a.toUpperCase();for(var b=Array(3),c=0,d,e,g=0;6>g;g+=2)d="0123456789ABCDEF".indexOf(a.charAt(g)),e="0123456789ABCDEF".indexOf(a.charAt(g+1)),b[c]=16*d+e,c++;return b};h.num2hex=function(a){for(var b="#",c,d,e=0;3>e;e++)c=a[e]/16,d=a[e]%16,b+="0123456789ABCDEF".charAt(c)+"0123456789ABCDEF".charAt(d);return b};x.prototype.addItem=function(a,b,c){function d(a){var b=this.value;b&&b.has_submenu&&
...
title:b.submenu.title,extra:b.submenu.extra,autoopen:c.autoopen});d=!1}d&&!g.lock&&g.close()}var g=this;c=c||{};var f=document.createElement("div");f.className="litemenu-entry submenu";var m=!1;if(null===b)f.classList.add("separator");else{f.innerHTML=b&&b.title?b.title:a;if(f.value=b)b.disabled&&(m=!0,f.classList.add("disabled")),(b.submenu||b.has_submenu)&&f.classList.add("has_submenu");"function"==typeof b?(f.dataset.value=a,f.onclick_callback=b):f.dataset.value=b;b.className&&(f.className+=" "+
b.className)}this.root.appendChild(f);m||f.addEventListener("click",e);c.autoopen&&f.addEventListener("mouseenter",d);return f};x.prototype.close=function(a,b){this.root.parentNode&&this.root.parentNode.removeChild(this.root);this.parentMenu&&!b&&(this.parentMenu.lock=!1,this.parentMenu.current_submenu=null,void 0===a?this.parentMenu.close():a&&!x.isCursorOverElement(a,this.parentMenu.root)&&x.trigger(this.parentMenu.root,"mouseleave",a));this.current_submenu&&this.current_submenu.close(a,!0);this.root.closing_timer&&
clearTimeout(this.root.closing_timer)};x.trigger=function(a,b,c,d){var e=document.createEvent("CustomEvent");e.initCustomEvent(b,!0,!0,c);e.srcElement=d;a.dispatchEvent?a.dispatchEvent(e):a.__events&&a.__events.dispatchEvent(e);return e};x.prototype.getTopMenu=function(){return this.options.parentMenu?this.options.parentMenu.getTopMenu():this};x.prototype.getFirstEvent=function(){return this.options.parentMenu?this.options.parentMenu.getFirstEvent():this.options.event};x.isCursorOverElement=function(a,
b){var c=a.clientX,d=a.clientY,e=b.getBoundingClientRect();return e?d>e.top&&d<e.top+e.height&&c>e.left&&c<e.left+e.width?!0:!1:!1};h.ContextMenu=x;h.closeAllContextMenus=function(a){a=a||window;a=a.document.querySelectorAll(".litecontextmenu");if(a.length){for(var b=[],c=0;c<a.length;c++)b.push(a[c]);for(c=0;c<b.length;c++)b[c].close?b[c].close():b[c].parentNode&&b[c].parentNode.removeChild(b[c])}};h.extendClass=function(a,b){for(var c in b)a.hasOwnProperty(c)||(a[c]=b[c]);if(b.prototype)for(c in b.prototype)b.prototype.hasOwnProperty(c)&&
!a.prototype.hasOwnProperty(c)&&(b.prototype.__lookupGetter__(c)?a.prototype.__defineGetter__(c,b.prototype.__lookupGetter__(c)):a.prototype[c]=b.prototype[c],b.prototype.__lookupSetter__(c)&&a.prototype.__defineSetter__(c,b.prototype.__lookupSetter__(c)))};I.sampleCurve=function(a,b){if(b){for(var c=0;c<b.length-1;++c){var d=b[c],e=b[c+1];if(!(e[0]<a)){c=e[0]-d[0];if(1E-5>Math.abs(c))return d[1];c=(a-d[0])/c;return d[1]*(1-c)+e[1]*c}}return 0}};I.prototype.draw=function(a,b,c,d,e,g){if(c=this.points){this.size=
b;var f=b[0]-2*this.margin;b=b[1]-2*this.margin;e=e||"#666";a.save();a.translate(this.margin,this.margin);d&&(a.fillStyle="#111",a.fillRect(0,0,f,b),a.fillStyle="#222",a.fillRect(0.5*f,0,1,b),a.strokeStyle="#333",a.strokeRect(0,0,f,b));a.strokeStyle=e;g&&(a.globalAlpha=0.5);a.beginPath();for(d=0;d<c.length;++d)e=c[d],a.lineTo(e[0]*f,(1-e[1])*b);a.stroke();a.globalAlpha=1;if(!g)for(d=0;d<c.length;++d)e=c[d],a.fillStyle=this.selected==d?"#FFF":this.nearest==d?"#DDD":"#AAA",a.beginPath(),a.arc(e[0]*
f,(1-e[1])*b,2,0,2*Math.PI),a.fill();a.restore()}};I.prototype.onMouseDown=function(a,b){var c=this.points;if(c&&!(0>a[1])){var d=this.size[0]-2*this.margin,e=this.size[1]-2*this.margin,g=a[0]-this.margin,f=a[1]-this.margin;this.selected=this.getCloserPoint([g,f],30/b.ds.scale);-1==this.selected&&(d=[g/d,1-f/e],c.push(d),c.sort(function(a,b){return a[0]-b[0]}),this.selected=c.indexOf(d),this.must_update=!0);if(-1!=this.selected)return!0}};I.prototype.onMouseMove=function(a,b){var c=this.points;if(c){var d=
this.selected;if(!(0>d)){var e=(a[0]-this.margin)/(this.size[0]-2*this.margin),g=(a[1]-this.margin)/(this.size[1]-2*this.margin);this._nearest=this.getCloserPoint([a[0]-this.margin,a[1]-this.margin],30/b.ds.scale);var f=c[d];if(f){var m=0==d||d==c.length-1;!m&&(-10>a[0]||a[0]>this.size[0]+10||-10>a[1]||a[1]>this.size[1]+10)?(c.splice(d,1),this.selected=-1):(f[0]=m?0==d?0:1:Math.clamp(e,0,1),f[1]=1-Math.clamp(g,0,1),c.sort(function(a,b){return a[0]-b[0]}),this.selected=c.indexOf(f),this.must_update=
!0)}}}};I.prototype.onMouseUp=function(a,b){this.selected=-1;return!1};I.prototype.getCloserPoint=function(a,b){var c=this.points;if(!c)return-1;b=b||30;for(var d=this.size[0]-2*this.margin,e=this.size[1]-2*this.margin,g=c.length,f=[0,0],m=1E6,h=-1,k=0;k<g;++k){var l=c[k];f[0]=l[0]*d;f[1]=(1-l[1])*e;l=vec2.distance(a,f);l>m||l>b||(h=k,m=l)}return h};h.CurveEditor=I;h.getParameterNames=function(a){return(a+"").replace(/[/][/].*$/gm,"").replace(/\s+/g,"").replace(/[/][*][^/*]*[*][/]/g,"").split("){",
...
(function(B){function n(){this.addInput("in","*");this.size=[80,30]}function u(){this.addInput("in");this.addOutput("out");this.size=[80,30]}function l(){this.addInput("in");this.addOutput("out")}function r(){this.addInput("in","number",{locked:!0});this.addOutput("out","number",{locked:!0});this.addOutput("clamped","number",{locked:!0});this.addProperty("in",0);this.addProperty("in_min",0);this.addProperty("in_max",1);this.addProperty("out_min",0);this.addProperty("out_max",1);this.size=[120,50]}
function v(){this.addOutput("value","number");this.addProperty("min",0);this.addProperty("max",1);this.size=[80,30]}function k(){this.addInput("in","number");this.addOutput("out","number");this.addProperty("min",0);this.addProperty("max",1);this.addProperty("smooth",!0);this.addProperty("seed",0);this.addProperty("octaves",1);this.addProperty("persistence",0.8);this.addProperty("speed",1);this.size=[90,30]}function E(){this.addOutput("out","number");this.addProperty("min_time",1);this.addProperty("max_time",
2);this.addProperty("duration",0.2);this.size=[90,30];this._blink_time=this._remaining_time=0}function y(){this.addInput("in","number");this.addOutput("out","number");this.size=[80,30];this.addProperty("min",0);this.addProperty("max",1)}function A(){this.properties={f:0.5};this.addInput("A","number");this.addInput("B","number");this.addOutput("out","number")}function x(){this.addInput("in","number");this.addOutput("out","number");this.size=[80,30]}function I(){this.addInput("in","number");this.addOutput("out",
"number");this.size=[80,30]}function h(){this.addInput("in","number");this.addOutput("out","number");this.size=[80,30]}function L(){this.addInput("in","number");this.addOutput("out","number");this.size=[80,30];this.properties={A:0,B:1}}function G(){this.addInput("in","number",{label:""});this.addOutput("out","number",{label:""});this.size=[80,30];this.addProperty("factor",1)}function z(){this.addInput("v","boolean");this.addInput("A");this.addInput("B");this.addOutput("out")}function K(){this.addInput("in",
"number");this.addOutput("out","number");this.size=[80,30];this.addProperty("samples",10);this._values=new Float32Array(10);this._current=0}function C(){this.addInput("in","number");this.addOutput("out","number");this.addProperty("factor",0.1);this.size=[80,30];this._value=null}function H(){this.addInput("A","number,array,object");this.addInput("B","number");this.addOutput("=","number");this.addProperty("A",1);this.addProperty("B",1);this.addProperty("OP","+","enum",{values:H.values});this._func=
