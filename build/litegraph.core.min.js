...
$jscomp.polyfill=function(p,v,w,l){if(v){w=$jscomp.global;p=p.split(".");for(l=0;l<p.length-1;l++){var x=p[l];x in w||(w[x]={});w=w[x]}p=p[p.length-1];l=w[p];v=v(l);v!=l&&null!=v&&$jscomp.defineProperty(w,p,{configurable:!0,writable:!0,value:v})}};$jscomp.polyfill("Object.is",function(p){return p?p:function(v,p){return v===p?0!==v||1/v===1/p:v!==v&&p!==p}},"es6","es3");
...
$jscomp.owns=function(p,v){return Object.prototype.hasOwnProperty.call(p,v)};$jscomp.polyfill("Object.assign",function(p){return p?p:function(v,p){for(var l=1;l<arguments.length;l++){var w=arguments[l];if(w)for(var z in w)$jscomp.owns(w,z)&&(v[z]=w[z])}return v}},"es6","es3");$jscomp.SYMBOL_PREFIX="jscomp_symbol_";$jscomp.initSymbol=function(){$jscomp.initSymbol=function(){};$jscomp.global.Symbol||($jscomp.global.Symbol=$jscomp.Symbol)};
...
$jscomp.polyfill("Object.values",function(p){return p?p:function(v){var p=[],l;for(l in v)$jscomp.owns(v,l)&&p.push(v[l]);return p}},"es8","es3");$jscomp.polyfill("String.prototype.startsWith",function(p){return p?p:function(v,p){var l=$jscomp.checkStringArgs(this,v,"startsWith");v+="";var w=l.length,z=v.length;p=Math.max(0,Math.min(p|0,l.length));for(var m=0;m<z&&p<w;)if(l[p++]!=v[m++])return!1;return m>=z}},"es6","es3");
$jscomp.findInternal=function(p,v,w){p instanceof String&&(p=String(p));for(var l=p.length,x=0;x<l;x++){var z=p[x];if(v.call(w,z,x,p))return{i:x,v:z}}return{i:-1,v:void 0}};$jscomp.polyfill("Array.prototype.findIndex",function(p){return p?p:function(p,w){return $jscomp.findInternal(this,p,w).i}},"es6","es3");
