(function(global) {
	...
	class LGraphPoints3D {
		...
		updatePoints() {
			...
			this.points = LGraphPoints3D.generatePoints(radius, num_points, mode, this.points, this
				.normals, this.properties.regular, obj);
			...
			if (regular) {
				if (mode == LGraphPoints3D.RECTANGLE) {
					...
					for (var i = 0; i < side; ++i)
						for (var j = 0; j < side; ++j) {
							var pos = i * 3 + j * 3 * side;
							polarToCartesian(temp, (i / side) * 2 * Math.PI, ((j / side) - 0.5) * 2 *
								Math.PI, radius);
							points[pos] = temp[0];
							points[pos + 1] = temp[1];
							points[pos + 2] = temp[2];
		...
		static generateFromObject(points, normals, size, obj, evenly) {
			...
			for (var i = 0; i < size; i += 3) {
				var r = Math.random();
				var index = evenly ? LGraphPoints3D.#findRandomTriangle(areas, r) : Math.floor(r *
					num_triangles);
				//get random triangle
				var a = 0;
				var b = 0;
				...
				if (normals && mesh_normals) {
					...
					normals[i + 1] = af * mesh_normals[a + 1] + bf * mesh_normals[b + 1] + cf *
						mesh_normals[c + 1];
					normals[i + 2] = af * mesh_normals[a + 2] + bf * mesh_normals[b + 2] + cf *
						mesh_normals[c + 2];
	...
	class LGraphGeometryTransform {
		...
		onExecute() {
			...
			if (this.must_update || geo._id != this._last_geometry_id || geo._version != this
				._last_version || key != this._last_key) {
	...
	class LGraphGeometryExtrude {
		...
		extrudeGeometry(geo) {
			...
			if (geo.type == "line_loop") {
				var new_vertices = new Float32Array(num_points * 6 *
				3); //every points become 6 ( caps not included )
				var npos = 0;
				for (var i = 0, l = vertices.length; i < l; i += 3) {
					tempA[0] = vertices[i];
	...
	class LGraphGeometryEval {
		...
		onExecute() {
			...
			if (this.geometry_id != geometry._id || this.version != geometry._version || this
				.must_update || this.properties.execute_every_frame) {
				this.must_update = false;
	...
	*/
			...
			if (this.geometry_id != geometry._id || this.version != geometry._version || this
				...
				for (var i = 0; i < l; i += 3) {
					...
					for (var j = i + 3; j < l; j += 3) {
						...
						var y2 = vertices[j + 1];
						var z2 = vertices[j + 2];
						var dist = Math.sqrt((x - x2) * (x - x2) + (y - y2) * (y - y2) + (z - z2) * (
							z - z2));
						if (dist > max_dist || dist < min_dist || (probability < 1 && probability <
								Math.random()))
							continue;
		...
		updateMesh(geometry) {
			...
			for (var i in geometry) {
				...
				if (!mesh_buffer || mesh_buffer.data.length != buffer_data.length) {
					mesh_buffer = new GL.Buffer(i == "indices" ? GL.ELEMENT_ARRAY_BUFFER : GL
						.ARRAY_BUFFER, buffer_data, spacing, GL.DYNAMIC_DRAW);
				}
				else {
			...
			if (this.mesh.vertexBuffers.normals && this.mesh.vertexBuffers.normals.data.length != this
				.mesh.vertexBuffers.vertices.data.length) {
	...
	class LGraphRenderMesh {
		...
		onExecute() {
			...
			LiteGraph.LGraphRender.onRequestCameraMatrices(view_matrix, projection_matrix,
			...
			if (texture) {
				...
				if (!shader)
					shader = gl.shaders["textured"] = new GL.Shader(LGraphRenderPoints
						.vertex_shader_code, LGraphRenderPoints.fragment_shader_code, {
							USE_TEXTURE: ""
						});
			}
			else {
				...
				if (!shader)
					shader = gl.shaders["flat"] = new GL.Shader(LGraphRenderPoints.vertex_shader_code,
	...
	class LGraphGeometryPrimitive {
		...
		onExecute() {
			...
			//update
			if (this.last_info.type != this.properties.type || this.last_info.size != size || this
				.last_info.subdivisions != this.properties.subdivisions)
	...
	class LGraphRenderPoints {
		...
		updateMesh(geometry) {
			...
			if (!this.buffer || !this.buffer.data || this.buffer.data.length != geometry.vertices
				.length)
		...
		onExecute() {
			...
			if (texture) {
				...
				if (!shader)
					shader = gl.shaders["textured_points"] = new GL.Shader(LGraphRenderPoints
						.vertex_shader_code, LGraphRenderPoints.fragment_shader_code, {
							USE_TEXTURED_POINTS: ""
						});
			}
				...
				if (!shader)
					shader = gl.shaders["points"] = new GL.Shader(LGraphRenderPoints
						.vertex_shader_code, LGraphRenderPoints.fragment_shader_code, {
							USE_POINTS: ""
						});
			}
